...
public class UserController {
    ...
    @PostMapping("/list")
    public Result list(@RequestParam(defaultValue = "1") Integer currentPage, @RequestParam(defaultValue = "5") Integer pageSize, @RequestBody User user){
        Page page = new Page(currentPage,pageSize);
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        String name = user.getName();
        String mobile = user.getMobile();
        if(StringUtils.isEmpty(name)&&StringUtils.isEmpty(mobile)){
            IPage pageData = userService.page(page, new QueryWrapper<User>().orderByDesc("user_id"));
